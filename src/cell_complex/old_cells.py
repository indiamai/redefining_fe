    ...
    * TODO: make this actually an abstract class?
    ...
    def get_immersion_operator(
        ...
        for i in range(output_level - initial_level):
            ops = [(upper, self.edges[upper, current_entity][0]) for upper in range(
                len(self.points)) if (upper, current_entity) in self.edges]
    ...
    def plot(self):
        ...
        for (u, l) in self.edges:
            ...
            plt.annotate(self.points[l].group.perm_name(
                self.edges[u, l][1]), ((x1 + x2) / 2, (y1 + y2) / 2))
...
class Interval(Point):
    ...
    def default_cell_complex(self):
        ...
        cell.add_point(Vertex())
        def i_0(v): return v(-1)
        def i_1(v): return v(1)
...
class Triangle(Point):
    ...
    def default_cell_complex(self):
        ...
        cell.add_point(Interval())
        r = Permutation([1, 0])
        def i_v0(v): return v(-1)
        def i_v1(v): return v(1)
        def i_e0(v): return (lambda x: v(x, -np.sqrt(3) / 3))

        def i_e1(v): return (lambda a: v(-1 + (a + 1) / 2, -
                                         np.sqrt(3) / 3 + ((a + 1) / 2) * np.sqrt(3)))
        def i_e2(v): return (lambda a: v(
            1 - (a + 1) / 2, -np.sqrt(3) / 3 + ((a + 1) / 2) * np.sqrt(3)))
